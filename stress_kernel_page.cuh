...
__global__ void
memoryKernel(unsigned int *k_ptrs[MAX_SPACES], unsigned long long *k_result, int bytesize, unsigned long long run_time,
                 int myZero, unsigned int *c_flush, unsigned int* starting)  
{
    ...
    unsigned int start_idx = starting[(gbl_blk * NUM_WARPS) + lcl_wrp];
    int wrp_log;
    wrp_log =  ((gbl_blk * NUM_WARPS) + lcl_wrp) * wrp_max;
    ...
    //    for (k = 0; k < NUM_SPACES; k++) {
        ...
        //   for (i = 0; i < NUM_PASSES; i++) {
	      // compute the local warp number and the start index in its array partition
            ...
            ptr = start_idx;
            // int base = ((gbl_blk * NUM_WARPS) + lcl_wrp) * wrp_max;
              //ptr = ptr_start;
            //   __syncthreads();
            //   before = clock64();
...
//    } //end outer loop for run time
    ...
    k_data[2] = r_sum + start_idx;
    // if((gbl_blk * NUM_WARPS) + lcl_wrp == 1)
    k_data[3] = start_idx;
}
