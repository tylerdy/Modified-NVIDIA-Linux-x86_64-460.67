...
memoryKernel(unsigned int *k_ptrs[MAX_SPACES], unsigned short *k_result, int bytesize, unsigned long long run_time, int myZero, unsigned int *c_flush)  //c_flush is size of k_data (bytesize) and used to flush cache initially
{
     ...
     */
    extern __shared__ unsigned long long cycles_before;
    int wrp_log;
    wrp_log =  ((gbl_blk * NUM_WARPS) + lcl_wrp) * wrp_max;
    int log_idx;

    int ptr;  // holds the index of the next array element to be read
    extern __shared__ int ptr_start;
    ptr_start = ((gbl_blk * NUM_WARPS) + lcl_wrp) * (wrp_max * 32) + (ptr * myZero * wrp_count);
    extern __shared__ unsigned int r_sum;  //a nonsense variable used to help defeat optimization
    r_sum = 0;
      ...
      // for (k = 0; k < NUM_SPACES; k++) {
	      ...
	      // uncomment the lines inside the loop to record read access times in shared memory
              for (wrp_count = 0; wrp_count < wrp_max; wrp_count++) {
                 ...
                 ptr = k_data[ptr];  // the actual pointer chase
                 blk_log[wrp_log + wrp_count] = (unsigned short) (clock64() - cycles_before);
               } //end loop for pointer chasing
/*
...
//#ifdef DO_LOG	       
               ...
               for (j = 0; j < wrp_max; j++)
                  k_result[log_idx + wrp_log + j] = blk_log[wrp_log + j];
//#endif		  
