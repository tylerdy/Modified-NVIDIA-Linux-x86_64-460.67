...
template <int BLOCK_SIZE> __global__ void MatrixMulCUDA(float *C, float *A,
                                                        float *B, int wA,
                                                        int wB, unsigned long long run_time) {
    /*int smid;
    asm("mov.u32 %0, %smid;" : "=r"(smid));
    if(smid >= num_sm) { // if l1 we may have multiple blocks per SM but dont want to kill any
        if(threadIdx.x==0 && threadIdx.y==0)printf("block returned on smid %d\n",smid);
        return;
    }*/
...
int MatrixMultiply(int argc, char **argv,
                   ...
                   const dim3 &dimsB, unsigned long long run_time) {
    ...
    float *start_addr;
    checkCudaErrors(cudaMalloc(reinterpret_cast<void **>(&start_addr), mem_size_A + mem_size_B + mem_size_C));
    d_A = start_addr;
    d_B = (float*)((void *)d_A + mem_size_A);
    // d_B = d_A + 1;
    d_C = (float*)((void *)d_B + mem_size_B);
    // printf("%016x, %016x, %016x， %016x\n", d_A, d_B, d_C, mem_size_A);
    // return 0;
    ...
    // checkCudaErrors(cudaMemcpyAsync(h_C, d_C, mem_size_C, cudaMemcpyDeviceToHost, 0));
    // checkCudaErrors(cudaStreamSynchronize(stream));
...
int main(int argc, char **argv) {
    ...
    dim3 dimsB(5 * block_size, 4 * block_size, 1); // enough to get 5*4=20 blocks, one per SM (compute, so array size doesnt matter)
    ...
    printf("MatrixA(%d,%d), MatrixB(%d,%d)\n", dimsA.x, dimsA.y,
                                               dimsB.x, dimsB.y);
    int matrix_result = MatrixMultiply(argc, argv, block_size, dimsA, dimsB, run_time);
