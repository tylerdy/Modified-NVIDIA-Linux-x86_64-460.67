...
int main(int argc, char *argv[])
{
    ...
    void *saddr;
    checkCudaErrors(cudaMalloc((void **)&saddr, DATA_SIZE + DATA_SIZE));
    d_Kernel = (float*)(saddr);
    d_Data = (float*)((void *)d_Kernel + DATA_SIZE);
    fprintf(stderr, "%llu\n", ((((unsigned long long)saddr) >> 12) << 12) - (1<<21));
    ...
    fwtBatchGPU(d_Data, 1, log2Data, run_time);
    /* we only use stress from one invocation
    fwtBatchGPU(d_Kernel, 1, log2Kernel);
