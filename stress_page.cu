...
#include "stress_kernel_page.cuh" 
...
int main(int argc, char *argv[])
{
  ...
  unsigned long long *d_result;  //device memory array to hold logged values
  ...
  unsigned long long *h_result;  //host memory array to hold logged values
  
  unsigned int *h_starting, *d_starting;
  ...
  // read from files
  int *ptrs;
  FILE *kernelFP;           //file containing generated randomized pointer-chasing data
  char new_line[512];
  ptrs = (int *)malloc(element_count * sizeof(int));
  kernelFP = stdin;  //read by redirecting stdin to file
  //read the file into the allocated memory 
  i = 0;
  while (!feof (kernelFP)) {
    if (fgets (new_line, sizeof(new_line), kernelFP) == NULL)
        break;
    if (i == element_count) {
       printf("Error -- File too large\n");
       exit(-1);
    }
    sscanf(new_line, "%d", &ptr);
    ptrs[i] = ptr;
    i++;
  }
  ...
  checkCudaErrors(cudaMallocHost(&h_data, CONTIG_SIZE));

  checkCudaErrors(cudaMallocHost(&h_starting, NUM_BLOCKS*NUM_WARPS * sizeof(unsigned int)));
  checkCudaErrors(cudaMalloc((void **) &d_starting, NUM_BLOCKS*NUM_WARPS * sizeof(unsigned int)));
  ...
  for(i = 0; i < wrpcnts;i++){
    h_starting[i] = ptrs[i * element_count / wrpcnts];
    // h_starting[i]=i*array_count/wrpcnts;
    // printf("%d, %d\n",i,h_starting[i]);
  }
  // printf("%d\n", h_data[65504]);
// return 0;
  ...
  for (i = 0; i < NUM_SPACES; i++) {
     checkCudaErrors(cudaMemcpyAsync(h_ptrs[i],  h_data, CONTIG_SIZE , cudaMemcpyHostToDevice, my_stream)); 
     checkCudaErrors(cudaStreamSynchronize(my_stream));
  }
  // printf("%d\n", h_starting[2]);
  checkCudaErrors(cudaMemcpyAsync(d_starting,  h_starting, NUM_BLOCKS*NUM_WARPS * sizeof(unsigned int),
  ...
  memoryKernel<<<Blocks, Threads, 0, my_stream>>>(d_ptrs, d_result, bytesize, run_time, 0, d_flush, d_starting);  
